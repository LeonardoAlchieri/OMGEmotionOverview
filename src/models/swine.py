...
class SwinTEmo(nn.Module):
    def __init__(self):
        super().__init__()
        self.backbone = swin_3d_tiny()
    ...
    def forward(self, x):
        ...
        x = self.backbone(x)
        x_val = self.head_val(x)
        x_aro = self.head_aro(x)
        return x
...
class dualHead(nn.Module):
    def __init__(self, in_channels: int, hidden_channels: int):
        super().__init__()
        self.head_val = Head(in_channels=in_channels, hidden_channels=hidden_channels)
        self.head_aro = Head(in_channels=in_channels, hidden_channels=hidden_channels)
